...
const TokenAABI = require("./../abis/Token.json"); // Asegúrate de tener el ABI correcto

// Configuración inicial
const provider ="https://scroll-sepolia.g.alchemy.com/v2/-Om5pWvbQTGynmGJHrCUkiXa-ES1ZVuW";
const signer = provider.getSigner();
const tokenAAddress = "0xbbbdec7784e51c80a6b86e80f6e8f7a313460906"; // Reemplaza con la dirección del contrato TokenA
...
async function approve(spender, amount) {
  try {
    const tx = await tokenAContract.approve(spender, ethers.utils.parseUnits(amount, 18));
...
async function transfer(recipient, amount) {
  try {
    const tx = await tokenAContract.transfer(recipient, ethers.utils.parseUnits(amount, 18));
    console.log("Transacción enviada", tx.hash);
    await tx.wait();
...
async function transferFrom(from, to, amount) {
  try {
    const tx = await tokenAContract.transferFrom(from, to, ethers.utils.parseUnits(amount, 18));
    console.log("Transacción enviada", tx.hash);
    await tx.wait();
